...
export const UniversitasProvider = props => {
    ...
    const Action = ({ Id }) => {
        ...
        const handleDelete = (event) => {
            ...
            axios.delete(`https://admin-api-kaderisasi-dev.salmanitb.com/v1/universities/${Id}`)
                .then((res) => {
                    let dataBaru = rows.filter(e => { return e.id !== Id })
                    setRows([...dataBaru])
                })
        }
        ...
        return (
            <>
                <Button size="small" className="edit-button buttons" variant="contained" color="secondary"><Link to={`/PerguruanTinggi/form-universitas/${Id}`} className="edit">Edit</Link></Button>
                <Button onClick={handleDelete} size="small" className="delete-button" variant="contained" color="secondary"><Delete fontSize="small" />Hapus</Button>
            </>
        )
    }
    ...
    return (
        <UniversitasContext.Provider value={{
            rows,
            setRows,
            universitiesState,
